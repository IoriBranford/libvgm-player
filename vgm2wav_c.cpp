...
int main(int argc, const char *argv[]) {
    ...
    player = PlayerC_NewVGM();

    /* setup the player's output parameters and allocate internal buffers */
    if (PlayerC_SetOutputSettings(player, sample_rate, 2, bit_depth, BUFFER_LEN)) {
    ...
    if(PlayerC_LoadFile(player, argv[0])) {
        fprintf(stderr,"failed to load file\n");
        return 1;
    }
    plrEngine = player->player->GetPlayer();

    if (plrEngine->GetPlayerType() == FCC_VGM)
    {
        VGMPlayer* vgmplay = dynamic_cast<VGMPlayer*>(plrEngine);
        PlayerC_SetLoopCount(player, vgmplay->GetModifiedLoopCount(loops));
    }
    ...
    while(totalFrames) {
        ...
        PlayerC_Render(player, curFrames * ((bit_depth / 8) * 2),packed);
